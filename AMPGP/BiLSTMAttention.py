...
class BiLSTMAttention(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_heads,dropout,extract_dim):
        super(BiLSTMAttention, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers

        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True,bidirectional=True)
        self.self_attention = SelfAttention(hidden_size*2, num_heads)
        self.dropout=nn.Dropout(dropout)
        self.fc1 = nn.Linear(hidden_size*2, extract_dim)
...
model = BiLSTMAttention(input_size, hidden_size, num_layers, num_heads,dropout,extract_dim).to(device)
...
for train_index, val_index in skf.split(X, y):
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save(model, './Pout/BiLSTMAttention.pth')
