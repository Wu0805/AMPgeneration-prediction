...
MAX_SEQ_LEN = 50
data = pd.read_csv('./data/generate_model_data/CAMP.csv', skiprows = 1, usecols = range(2), header=None, names=['Id','seq'])
all_sequences = np.asarray(data['seq'])


CHARACTER_DICT = {
	'A': 1, 'C': 2, 'D': 3, 'E': 4, 'F': 5, 'I': 6, 'H': 7,
	'K': 8, 'M': 9, 'L': 10, 'N': 11, 'Q': 12, 'P': 13, 'S': 14,
	'R': 15, 'T': 16, 'W': 17, 'V': 18, 'Y': 19, 'G': 20}
INDEX_DICT = {
	1: 'A', 2: 'C', 3: 'D', 4: 'E', 5: 'F', 6: 'I', 7: 'H',
...
for i in range(len(all_sequences)):
	sequences.append(all_sequences[i].replace('\t',''))
all_data = []
for i in range(len(sequences)):
    all_data.append(sequence_to_vector(sequences[i]))
...
class Generator(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, max_seq_len, gpu=True, oracle_init=False):
        ...
        self.selfAttention = SelfAttention(embedding_dim, 1)  # Add self-attention layer
    ...
    def forward(self, inp, hidden):
        ...
        emb = self.selfAttention(emb, emb, emb)
    ...
    def batchNLLLoss(self, inp, target):
        ...
        return loss/batch_size  # per batch
...
class Discriminator(nn.Module):
	...
	def __init__(self, embedding_dim, hidden_dim, vocab_size, max_seq_len, gpu=True, dropout=0.2):
		...
		self.embeddings = nn.Embedding(vocab_size, embedding_dim)
		self.selfAttention = SelfAttention(embedding_dim, 1)
...
if __name__ == '__main__':
	...
	torch.save(gen.state_dict(), './model/' + gen_model)
	torch.save(dis.state_dict(), './model/' + dis_model)
